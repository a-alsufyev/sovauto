    ...
    <div className="pt-20">
          ...
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-8 items-center text-[10px] uppercase tracking-[0.2em] font-bold text-gray-400">
              <div className="flex items-center gap-3">
                ...
                <span>{language === 'ru' ? 'ШТАБ' : 'HQ'}: {city}, {language === 'ru' ? 'СССР' : 'USSR'}</span>
              </div>
              <div className="flex items-center gap-3">
                <Calendar size={16} className="text-accent" />
                <span>{language === 'ru' ? 'РАБОТА' : 'OPERATIONAL'}: {factory.years.start} – {factory.years.end || (language === 'ru' ? 'НАСТ. ВРЕМЯ' : 'PRESENT')}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 px-4 bg-bg">
        <div className="container mx-auto">
          <div className="mb-12">
            <span className="text-accent text-[10px] font-bold uppercase tracking-widest mb-2 block">{language === 'ru' ? 'История производства' : 'Production History'}</span>
            <h2 className="font-display text-4xl font-bold">{language === 'ru' ? 'Стандартный парк' : 'Standard Fleet'}</h2>
          </div>
          ...
          {factoryVehicles.length === 0 && (
            <div className="py-20 text-center opacity-40">
              <p className="text-sm uppercase tracking-widest">{language === 'ru' ? 'В основном архиве нет автомобилей, приписанных именно к этому заводу.' : 'No vehicles specifically tagged to this plant in the main archive.'}</p>
            </div>
          )}
        </div>
      </section>
